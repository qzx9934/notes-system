...
### `POST /api/notes/export-exam` —— 已选笔记生成 Word 试卷（登录用户）

请求：`{"ids":[1,2,3],"exam_title":"班前测试"}`。`exam_title` 是整份试卷的名称/抬头，不是笔记标题。返回以试卷标题命名的压缩包（如 `班前测试.zip`），内含两份 Word：`班前测试-无答案.docx` 和 `班前测试-含答案.docx`。试卷题面只包含序号和笔记标题，不写入编号、来源、星级、日期等参数；Word 使用宋体，试卷标题四号加粗，标题外其余内容均为小四、不加粗、1.0 倍行距；无答案版本会按笔记内容预留答题空位。
