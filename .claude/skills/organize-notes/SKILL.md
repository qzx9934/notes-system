...
| `title` | 是 | 笔记标题，能独立看懂、准确描述该知识点 |
| `content` | 否 | 内容详情，**数字/定值/条件原样保留**；有小标题或序号时注意换行；支持 Markdown；尽量不少于 200 字 |
...
> 整理原则：一条笔记可以包含多个要点但必须确实是一个知识点，同一件事故或案例可整理成一条笔记；笔记标题能独立看懂、能准确描述该知识点；数字、定值、条件务必原样保留；不要杜撰资料里没有的内容；如有小标题或序号要注意换行；每条笔记字数不限，尽量不少于 200 字。
