...
| 你要做的事 | 用这条命令（在仓库目录执行） |
|------------|------------------------------|
| 看有哪些章节 | `python .claude/skills/organize-notes/note_cli.py sections` |
| 批量上传笔记（**主用**，自动去重合并） | `python .claude/skills/organize-notes/note_cli.py import /tmp/notes.json` |
...
> **仅当上面子命令确实覆盖不到某个操作时**，才参照《API 接口速查》用 Python urllib 自行调用
...
| `source`　| 否　 | **仅这 10 项**：规程/规章制度/技术文件/技术通知/操作票/事故预案/事故通报/缺陷异常/培训/个人总结（其它值会被系统归一为"个人总结"） |
...
```bash
python .claude/skills/organize-notes/normalize_numbers.py /tmp/notes.json /tmp/notes_fixed.json
```
...
**校验差异**：
- 单条接口（POST/PUT 单条）：严格校验 `section`、`level`、`date`、`source`，非法返回 `400`
- 批量接口（ingest）：更宽容——非法的 `level`/`date`/`source` 自动回退默认值（`★` / 当天 / `个人总结`），但 **`section` 非法仍会被跳过**
...
### 3c · 首选：CLI 工具（覆盖增删改查，编码安全）

```bash
CLI=".claude/skills/organize-notes/note_cli.py"
...
> 上面 3a/3b 的 urllib 片段仅作为「脚本未覆盖某操作」时的兜底参考。
