...
def build():
    ...
    para(doc, '针对电厂集控运行专业知识分散、经验难以沉淀、新员工培养周期长等长期痛点，'
              ...
              '卡片漫游等实用功能，并创新性地接入 DeepSeek 大模型，实现笔记的 AI 总结、AI 填充、'
              'AI 整理与一键批量处理；同时设计了管理员 / 共建者 / 查看者三级权限与“提交—审批”'
    ...
    bullet(doc, '三类智能：依托大模型实现 AI 总结、AI 填充、AI 整理，把“整理资料”这一最耗时环节交给 AI。')
    ...
    make_table(doc,
               ...
               widths=[2.6, 10.2])
    para(doc, '后端共配套 90 项自动化测试用例，覆盖鉴权、字段校验、编号生成、审批流、AI 接口等关键路径，'
    ...
    para(doc, '将电厂运行知识划分为系统设备、运行操作、安全管理、技术标准、综合管理 5 大领域共 26 个子类，'
              ...
              '形成结构清晰、可检索、可分级的知识库。')
    heading(doc, '（二）AI 大模型深度赋能，让整理“省时省力”（核心创新）', 2)
    para(doc, '这是本成果区别于普通笔记工具的关键所在——把大模型嵌入知识沉淀的全流程：')
